...
export class ManageEmailComponent implements OnInit {
  ...
  getCategoriesWithSubCategories(filter) {
    this.marketingManagementService.getCategoriesWithSubcategories(this.searchCategoryText, filter).subscribe(result => {
  ...
  navigateToEditCategoryPage(category: getCategoriesWithSubcategoriesList) {
    this.router.navigate(['/marketing-management/create-blog-categories']);
