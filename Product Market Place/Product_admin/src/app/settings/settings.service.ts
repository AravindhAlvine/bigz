...
import { TaxSettings } from '../models/Settings';
...
export class SettingsService {
  ...
  gePaymentGatewayDetails(gatewayName: string): Observable<GeneralResponse<any[]>> {
    return this.http.get<GeneralResponse<any[]>>(environment.settings.getPaymentGatewayDetails.replace(':name', gatewayName))
      .pipe(
        map(result => result)
      );
  }

  savePaymentGatewaySettings(id:string, body: any): Observable<GeneralResponse<any>> {
    return this.http.put<GeneralResponse<any>>(environment.settings.savePaymentGatewayDetails.replace(':id', id), body)
      .pipe(
        map(result => result)
      );
  }

  getEmailSettingsDetails() : Observable<GeneralResponse<any>> {
    return this.http.get<GeneralResponse<any>>(environment.settings.getEmailSettings)
      .pipe(
        map(result => result)
      );
  }

  getTaxSettingsList(): Observable<GeneralResponse<TaxSettings[]>> {
    return this.http.get<GeneralResponse<TaxSettings[]>>(environment.settings.getTaxSettingsList)
      .pipe(
        map(result => result)
      );
  }

  deleteTaxSettingItemById(taxId: string): Observable<GeneralResponse<any>> {
    return this.http.delete<GeneralResponse<any>>(environment.settings.deleteTaxSettingItemById.replace(':tax_item_id', taxId))
      .pipe(
        map(result => result)
      );
  }
