...
import { VendorFilter } from 'src/app/models/Vendor';
...
export class ManageHairStylistsComponent implements OnInit {
  ...
  filterItems: VendorFilter = {} as VendorFilter;
  ...
  ngOnInit(): void {
    this.getVendorListAPICall(this.filterItems);
    this.getCategoryListAPICall();
  }

  getVendorListAPICall(filter) {
    this.hairStylistsService.getVendorList(filter).subscribe(result => {
