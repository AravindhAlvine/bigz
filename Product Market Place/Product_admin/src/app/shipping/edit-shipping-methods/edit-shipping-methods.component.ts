...
export class EditShippingMethodsComponent implements OnInit {
  ...
  getCategoryListAPICall() {
    this.shippingService.getCategoryList().subscribe(result => {
  ...
  createCategoryAPICall(request: Categories[]) {
    this.shippingService.createCategory(request).subscribe(result => {
  ...
  uploadCategoryAssetsAPICall() {
    this.shippingService.uploadCategoryAssets(this.uploadAssestsformData, this.currentCategoryId).subscribe(result => {
      if (this.isEditMode)
  ...
  uploadSubCategoryAssetsAPICall() {
    this.shippingService.uploadSubCategoryAssets(this.uploadAssestsformData, this.currentSubCategoryId).subscribe(result => {
      if (this.isEditMode)
  ...
  generateCategorySlugAPICall(title: string): Promise<string> {
    return new Promise((resolve, reject) => {
      this.shippingService.generateCategorySlug({ title: title }).subscribe(result => {
  ...
  generateSubCategorySlugAPICall(title: string): Promise<string> {
    return new Promise((resolve, reject) => {
      this.shippingService.generateSubCategorySlug({ title: title }).subscribe(result => {
        resolve(result.data.slug);
      })
    })
  }
  ...
  saveSubcategoriesAPICall(request: SubCategories[]) {
    this.shippingService.createSubCategories(request).subscribe(result => {
