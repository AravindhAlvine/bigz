...
export class MainCategoriesService {
   ...
   * }
   */
  getCategoriesWithSubcategories(searchText: string): Observable<GeneralResponse<getCategoriesWithSubcategoriesList[]>> {
    const url = environment.categories.getCategoriesWithSubCategories;
    const apiUrl = searchText ? `${url}?search=${searchText}` : url;
    return this.http.get<GeneralResponse<getCategoriesWithSubcategoriesList[]>>(apiUrl)
    .pipe(
      map(result => result)
    );
  }
